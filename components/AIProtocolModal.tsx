...
export const AIProtocolModal: React.FC<AIProtocolModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative glass-card w-full max-w-lg rounded-2xl shadow-2xl border border-[var(--md-outline-variant)] overflow-hidden flex flex-col max-h-[90vh]">

        <div className="px-6 py-4 border-b border-[var(--md-outline-variant)] flex justify-between items-center bg-[var(--md-surface-container)]">
          <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
            <Sparkles className="w-5 h-5" />
            <h2 className="font-semibold tracking-tight">AI Assistant</h2>
          </div>
          <button onClick={onClose} className="text-[var(--md-on-surface-variant)] hover:text-[var(--md-on-surface)] p-1 rounded-full hover:bg-[var(--md-surface-container)] transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          {step === 'input' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <textarea
                ...
                className="w-full h-32 p-4 rounded-xl bg-[var(--md-surface-container)] border border-[var(--md-outline-variant)] resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500/50 text-[var(--md-on-surface)] placeholder:text-[var(--md-on-surface-variant)]"
          ...
          {step === 'processing' && (
            <div className="flex flex-col items-center justify-center py-12 space-y-4 animate-in fade-in duration-300">
              <div className="relative">
                <div className="w-12 h-12 rounded-full border-4 border-[var(--md-surface-container)]"></div>
              ...
              <p className="text-sm font-medium text-[var(--md-on-surface-variant)] animate-pulse">Designing workflow...</p>
            </div>
          )}
          ...
          {step === 'review' && result && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              {result.type === 'protocol' && (
                <div className="glass-panel rounded-xl border border-[var(--md-outline-variant)] p-4 max-h-48 overflow-y-auto custom-scrollbar">
                  <h4 className="font-bold text-sm mb-2 text-[var(--md-on-surface)]">{result.data.title}</h4>
                  <ul className="space-y-2">
                    {result.data.steps.map((s, i) => (
                      <li key={i} className="text-xs text-[var(--md-on-surface-variant)] flex gap-2">
                        <span className="text-[var(--md-outline)] font-mono">{i + 1}.</span>
                        <span>{s.text}</span>
                        {s.action && <span className="px-1.5 py-0.5 bg-[var(--md-surface-container)] rounded text-[9px] uppercase font-bold tracking-wider">{s.action.type}</span>}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {result.type === 'experiment' && (
                <div className="p-3 glass-panel rounded-lg border border-[var(--md-outline-variant)]">
                  <span className="text-[10px] uppercase text-[var(--md-on-surface-variant)] font-bold tracking-wider">Target Volume</span>
                  <p className="text-sm font-mono font-medium text-[var(--md-on-surface)] mt-1">{result.data.targetVolume || '?'} mL</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="p-4 border-t border-[var(--md-outline-variant)] bg-[var(--md-surface-container)] flex justify-end gap-3">
          {step === 'review' ? (
            <>
              <button onClick={() => setStep('input')} className="px-4 py-2 text-sm font-medium text-[var(--md-on-surface-variant)] hover:text-[var(--md-on-surface)] transition-colors">Refine</button>
              <button onClick={handleCreate} className="px-5 py-2 bg-[var(--md-primary)] hover:brightness-110 text-[var(--md-on-primary)] rounded-xl text-sm font-medium shadow-md shadow-[var(--md-primary)]/20 flex items-center gap-2 transition-all active:scale-95">
